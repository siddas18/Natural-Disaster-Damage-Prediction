...
class RandomForest(QMainWindow):
    ...
    def update(self):
        ...
        if self.feature8.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_noaa[features_list[8]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_noaa[features_list[8]]],axis=1)

        if self.feature9.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_noaa[features_list[9]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_noaa[features_list[9]]],axis=1)

        if self.feature10.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_noaa[features_list[10]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_noaa[features_list[10]]],axis=1)

        if self.feature11.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_noaa[features_list[11]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_noaa[features_list[11]]], axis=1)

        if self.feature12.isChecked():
            if len(self.list_corr_features) == 0:
                self.list_corr_features = ff_noaa[features_list[12]]
            else:
                self.list_corr_features = pd.concat([self.list_corr_features, ff_noaa[features_list[12]]], axis=1)
...
class App(QMainWindow):
    ...
    def EDA1(self):
        ...
        dialog.m.ax.hist(X,bins = 1, density=True, facecolor='green', alpha=0.5)
